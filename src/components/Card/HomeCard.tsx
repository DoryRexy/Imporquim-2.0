...
export default function CardHome(props: Props) {
  return (
    ...
    >
      ...
      >
        <div
          className={` grid justify-center mb-6  overflow-auto ${props.textColor}`}
        >
          <SvgLoader
            alt={props.alt}
            src={props.img}
            classes="max-w-[120px] max-h-[120px] "
          />
        </div>
        {props.children}
        <div className="  flex justify-center  content-center h-20">
          <p
            className={` dark:text-white font-medium text-2xl text-center mx-8  ${props.textColor}`}
          >
            {props.titulo}
          </p>
        </div>
