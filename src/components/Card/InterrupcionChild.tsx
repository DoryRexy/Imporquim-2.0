...
export default function InterrupcionChild({
  state,
  title,
  start,
  end,
    ...
    <div className="">
      <div className="mx-4 ">
        ...
        <p className="mt-3 text-sm text-gray-700 flex gap-4 grid-flow-col">
          <strong>Localidades afectadas:</strong>
          {`${afectedTowns}`}
        </p>
      ...
      <div className=" grid  md:flex justify-center md:items-center my-4 ">
        <SvgLoader
          src="/img/suspensiones/timer.svg"
          alt="Imagen Reloj"
          key="timer-image"
        ></SvgLoader>
        ...
        <p className="text-md font-medium ml-4">{start}</p>
        <div className="text-md font-bold ml-4">
          <p>Fin:</p>
        </div>
        <p className="text-md font-medium ml-4">{end}</p>
      </div>
    </div>
  );
}
