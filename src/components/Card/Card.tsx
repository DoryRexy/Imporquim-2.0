...
export default function Card({ title, img, url, classes, state }: Props) {
  const stateColor =
    state == "active" ? "text-ice_verde-950" : "text-ice_azul-950";
  return (
    <>
      <Link
        href={url}
        className={`border rounded-xl h-56 w-56 sm:h-56 sm:w-56 grid justify-center 
      ...
      >
        <div className={`  `}>
          <div
            className={`grid justify-center content-end mt-3 ${stateColor} h-32 `}
          >
            <SvgLoader
              alt="Icono Card"
              src={img}
