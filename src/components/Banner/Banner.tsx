...
export default function Banner({
  ...
  return (
    <div
      className={`${classes} overflow-hidden lg:h-[27rem] xl:h-[30rem]   w-full flex flex-col 
            ${reverse ? " lg:flex-row-reverse" : "flex-col lg:flex-row"}`}
    >
      {/**DESCRIPCIONES */}
      <div
        className={`${borderClasses}   flex-1 flex ${
          reverse ? "xl:mr-40" : "lg:ml-16 xl:ml-40"
        }  content-center md:justify-self-center
      ...
      >
        <div className={`${borderClasses} mx-2 grid content-center  `}>
          <div className="flex flex-col gap-4 xl:flex-row my-4 justify-center  ">
            ...
            <span
              className={`${borderClasses} grid justify-center content-center text-center `}
            >
              <h1 className="text-4xl sm:text-5xl font-bold text-ice_azul-950">
                {title}
              </h1>
            </span>
          </div>
      ...
      {/**IMAGEN */}
      <div
        className={`${borderClasses} ${imgClasses} lg:w-[50%] xl:w-[55%] grid mt-4 md:mt-0`}
      >
        <img
          className=" inset-0 overflow-hidden grid "
          alt="Banner"
          src={img}
          width={finalWidth}
          height={finalHeight}
