  ...
  return (
    <div className=" bg-ice_gris-200 pb-10 ">
      <section id="banner" className={`${marginClasses} ${borderClasses} `}>
        ...
        <div
          className={`pl-12  bg-ice_gris-200 text-ice_azul-950 text-sm -translate-y-4 content-center pt-2 `}
        >
          <p>{date}</p>
        </div>
      </section>
      <section
        id="contenido"
        className={`${marginClasses} ${borderClasses} bg-white pb-6 `}
      >
        <div className={classNames(`${contentMargin}`)}>
          <h1 className="text-ice_azul-950 font-bold text-5xl py-8">{title}</h1>
          <p className="text-ice_azul-950 font-medium text-xl my-4">{resume}</p>
          <div className="grid font-sm text-justify gap-8 my-6">{children}</div>
        </div>
      </section>
    </div>
  );
