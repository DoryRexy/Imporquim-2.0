        ...
        <Disclosure as="div" key={id}>
          {({ open }) => (
            <>
              <Disclosure.Button
                key={`btn-${id}`}
                className="border-2 border-ice_gris-200 shadow-lg bg-white flex w-full justify-between rounded-sm font-medium
                         dark:bg-slate-700 
              ...
              <Transition show={open} {...dropDownTransitionClasses}>
                <Disclosure.Panel
                  static
                  className="px-4 pt-4 pb-2 bg-white text-sm text-ice_gris-950 dark:text-slate-100"
                >
                  <ul>
                    {content.map((item, index) => {
                      if (typeof item === "string") {
                        ...
                        return (
                          <ul
                            className="list-disc list-outside ml-5 my-5"
                            key={index}
                          >
